...
import { Upload, Download, FileSpreadsheet, Plus, RefreshCw } from 'lucide-react';
...
interface DataImporterProps {
  onDataImport: (data: StudentData[], mode: 'replace' | 'append') => void;
  hasData: boolean;
  existingData?: StudentData[];
}

export default function DataImporter({ onDataImport, hasData, existingData = [] }: DataImporterProps) {
  ...
  const handleFileUpload = (event: React.ChangeEvent<HTMLInputElement>) => {
    ...
    reader.onload = (e) => {
      try {
        ...
        const startIndex = importMode === 'append' && existingData.length > 0 
          ? existingData.length 
          : 0;

        const newStudents: StudentData[] = jsonData.map((row: any, index: number) => ({
          id: `S${String(startIndex + index + 1).padStart(3, '0')}`,
        ...
        onDataImport(newStudents, importMode);
        
        if (importMode === 'append') {
          setSuccess(`成功追加 ${newStudents.length} 条学生数据！当前共 ${startIndex + newStudents.length} 条`);
        } else {
          setSuccess(`成功导入 ${newStudents.length} 条学生数据！`);
        }
      } catch (err) {
    ...
    <div className="max-w-2xl mx-auto bg-white rounded-lg shadow p-6">
      ...
      {hasData && (
        <div className="bg-gray-50 p-4 rounded-lg mb-4">
          <p className="text-sm text-gray-600 mb-2">当前已有 {existingData.length} 条学生数据</p>
